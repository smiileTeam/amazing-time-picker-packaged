...
var AtpCoreService = (function () {
     ...
     * @return {?}
     */
    AtpCoreService.prototype.allowedTimes = function (min, max, system) {
        var /** @type {?} */ allTimes = [];
        ...
        for (var /** @type {?} */ i = nowMinHour; i <= nowMaxHour; i++) {
            ...
            for (j; j <= jDest; j++) {
                var /** @type {?} */ hour = (i <= 12 || system !== 'SYSTEM12' ? i : i - 12);
                var /** @type {?} */ minute = j;
                var /** @type {?} */ ampm = '';
                if (system === 'SYSTEM12') {
                    ampm = i < 12 ? 'AM' : 'PM';
     ...
     * @return {?}
     */
    AtpCoreService.prototype.ClockMaker = function (rayon, type, format) {
        var /** @type {?} */ r = rayon;
        var /** @type {?} */ step = type === 'hour' ? 1 : 5;
        var /** @type {?} */ j = r - 25;
        var /** @type {?} */ items = [];
     ...
     * @return {?}
     */
    AtpCoreService.prototype.drawCircle = function (rayon, j, start, length, step, type) {
        var /** @type {?} */ items = [];
        for (var /** @type {?} */ min = start; min < length; min += step) {
            var /** @type {?} */ str = String(min === 24 ? '0' : min);
            var /** @type {?} */ x = j * Math.sin(Math.PI * 2 * (min / (length - start)));
            var /** @type {?} */ y = j * Math.cos(Math.PI * 2 * (min / (length - start)));
            items.push({
                time: str,
                left: (x + rayon - 17) + 'px',
                top: (-y + rayon - 17) + 'px',
     ...
     * @return {?}
     */
    AtpCoreService.prototype.TimeToString = function (time, system) {
     ...
     * @return {?}
     */
    AtpCoreService.prototype.StringToTime = function (time, system) {
        var _d = time.split(':'), h = _d[0], m = _d[1];
        var /** @type {?} */ hour = +h > 12 && system === 'SYSTEM12' ? +h - 12 : +h;
    ...
    AtpCoreService.prototype.CalcDegrees = function (ele, parrentPos, step) {
        ...
        var /** @type {?} */ hitDistance = Math.sqrt(Math.pow(((ele.clientX - parrentPos.left) - targetX), 2) + Math.pow(((ele.clientY - parrentPos.top) - targetY), 2));
...
var TimePickerComponent = (function () {
    ...
    function TimePickerComponent(core$$1) {
        ...
        this.clockMaker = function () {
            ...
            _this.clockObject = _this.core.ClockMaker(_this.rayon, type, _this.config.system);
            _this.setArrow(null);
        };
        ...
        this.setArrow = function (obj) {
            ...
            _this.smallArrow = (_this.clockType === 'hour' && (time > 12 || time === 0));
        ...
        this.getDegree = function (ele) {
            ...
            if (_this.isClicked && (ele.currentTarget === ele.target || ele.target.nodeName === 'BUTTON')) {
                _this.smallArrow = false;
                var /** @type {?} */ clock = {
                    width: ele.currentTarget.offsetWidth,
                    height: ele.currentTarget.offsetHeight
                };
                var /** @type {?} */ clickHit = _this.core.CalcDegrees(ele, parrentPos, step);
                ...
                if (_this.clockType === 'hour') {
                    ...
                    if (_this.config.system === 'SYSTEM24' && clickHit.hitDistance < (_this.rayon * 0.60)) {
                        hour += 12;
                        if (hour === 24) {
                            hour = 0;
                        }
                        _this.smallArrow = true;
                    }
                }
                ...
                var /** @type {?} */ nowMinHour = (+min.split(':')[0] < 12 && _this.config.system === 'SYSTEM12') ? +min.split(':')[0] : +min.split(':')[0] - 12;
                var /** @type {?} */ nowMaxHour = (+max.split(':')[0] < 12 && _this.config.system === 'SYSTEM12') ? +max.split(':')[0] : +max.split(':')[0] - 12;
                var /** @type {?} */ nowMinMin = +min.split(':')[1];
                var /** @type {?} */ nowMaxMin = +max.split(':')[1];
                ...
                if (_this.allowed.indexOf(nowTime) > -1) {
                    ...
                    _this.rotationClass(clickHit.degrees);
                    _this.setActiveTime();
                }
    ...
    TimePickerComponent.prototype.GetTime = function () {
        var /** @type {?} */ time = this.core.TimeToString(this.time, this.config.system);
    ...
    TimePickerComponent.prototype.checkDisabled = function (t) {
        var /** @type {?} */ m = (this.clockType === 'minute') ? t : this.time.minute;
        var /** @type {?} */ h = (this.clockType === 'hour') ? t : this.time.hour;
        var /** @type {?} */ nowTime = this.GetNowTime(h, this.time.ampm, m);
    ...
    TimePickerComponent.prototype.GetNowTime = function (hour, ampm, minute) {
        var /** @type {?} */ Hour = (hour === 12 && ampm === 'AM' && this.config.system === 'SYSTEM12') ? '0' : hour;
        var /** @type {?} */ system = (this.config.system === 'SYSTEM12' ? ' ' + ampm : '');
        var /** @type {?} */ nowTime = Hour + ':' + minute + system;
        return nowTime;
    };
...
var FrenchPreference = {
    ...
    separator: ':',
    period: function (x) { return x === 'AM' ? 'Matin' : 'Après midi'; },
    clockHour: function (x) { return x; },
    clockMinute: function (x) { return x; },
    labels: {
        ok: 'Valider',
...
var defaultPreference = {
    ...
    separator: ':',
    period: function (x) { return x === 'AM' ? 'AM' : 'PM'; },
    clockHour: function (x) { return x; },
    clockMinute: function (x) { return x; },
    labels: {
        ok: 'ok',
...
var AtpDirective = (function () {
    ...
    AtpDirective.prototype.onClick = function (e) {
        ...
        var /** @type {?} */ system = ele.getAttribute('system') || 'SYSTEM12';
